// Package jwz implements Jamie Zawinski's mail-threading algorithm
// (https://www.jwz.org/doc/threading.html) for Go.
//
// Build groups a slice of EmailHeader values into Thread trees, joining
// messages by Message-ID / In-Reply-To / References, falling back to a
// locale-aware subject match when reference data is missing.
package jwz
...
// EmailHeader is the subset of RFC 5322 headers required for threading.
// EmailID is an application-defined identifier returned in the resulting
// Thread tree — it lets callers map nodes back to their own records.
...
// Thread is a fully-built conversation.
type Thread struct {
	Root     *ThreadNode
	LatestAt time.Time
	Count    int
...
// ThreadNode is one message inside a Thread.
...
// Build groups headers into threads. Output is deterministic: threads are
// sorted newest-first by LatestAt with EmailID as a stable tiebreaker, and
// children within a thread are sorted by Date.
